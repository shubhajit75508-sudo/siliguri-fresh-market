...
- **Profit reconciliation** (commit `1154f24`): growth/profit APIs return `partnerPayouts`, `netProfit`, `netMargin` (summary literal updated to pass tsc); analytics shows Net Profit KPI, "Partner Payouts (cost)" card, "Delivery Fees (shop profit)"; boy EarningsCard caption clarifies fee belongs to shop. Customer delivery fee = shop profit; boy earns flat `DELIVERY_COMMISSION` ₹40
- **Cancel route fix** (commit `7ce8696`): removed non-existent top-level `cancelled_at` column from `/api/delivery/cancel` (orders table has no such column — timestamps live in `address_snapshot.json`)
- **Play API 36 compliance** (commits `914be6d`, `083574f`): TWA app (`android-twa/`, applyId `com.siligurifreshmart`) bumped to compileSdk/targetSdk 36, versionCode 6, versionName 3.3.0; signed AAB built; upload key reset approved — new key (SHA1 B6:93:1A:…) valid **Sep 15, 2026 2:47 PM UTC**; cert exported to `android-twa/android/upload-key-cert.pem`. Real signing creds live in `android-twa/android/key.properties` (`storeFile=../signing.keystore`, alias `freshmart-upload`) — read programmatically, docs' old values were wrong
- **Same-day delivery 9 AM–4 PM** (commit `131b78e`): slots order-before-9AM→9–12, before-12PM→12–4; store hours 9–16; consistent across home-faqs, landing translations EN/BN/HI, zones, fish subcat SEO, shipping policy + schema, bulk TIME_SLOTS, about, footer
- **Phone numbers updated** (commit `85f3420`): `+91 9832966112` → `+91 96354 80453` everywhere; 3rd number `+91 62959 53287` added (footer, FAQs, shipping policy, checkout links, bulk wa.me fallbacks via `STORE_PHONE_2=919635480453`, `STORE_PHONE_3=916295953287`)
- **Site-wide rebrand → "Siliguri Freshmart"** (commit `77fa53a`): one word everywhere — src/, public/ (incl. sw.js, reviews.json, llms.txt), logo in header/footer/splash alts, docs, supabase SQL, android-twa strings.xml + settings.gradle, capacitor config. No "Siliguri Fresh Mart" / "Fresh Mart" matches remain
- **Product page redesign** (`src/app/(shop)/product/[slug]/product-client.tsx`): premium layout — rounded image card w/ hover zoom + gallery thumbnails, offer strip, ratings + "X bought today", delivery timeline, benefits list, Verified Seller + savings pills, qty stepper + Buy Now (desktop), nutrition + trust badges; Flipkart-style **sticky mobile buy bar** (thumb + live price/savings + Add to Cart) fixed at `bottom-[76px]`, above BottomNav; page wrapper adds `pb-44` on mobile so content clears the bar
- **Expandable sticky buy bar** (commit `e57dbeb`): compact bar = thumb + live total (price × qty) + savings + qty stepper + quick Add; tapping opens animated options panel containing weight pills, cut preference, cleaning options, total + Buy Now (shares state with desktop buy zone, scrolls at max-h 50vh)
