...
export function ProductCard({ product, variant = "default", badge }: ProductCardProps) {
  ...
  return (
    <Link href={`/product/${product.slug}`} className="group block h-full">
      ...
      >
        <div className="relative aspect-[5/4] overflow-hidden bg-neutral-950">
          <Image
            src={product.image}
            alt={product.name}
            fill
          ...
          {tag && (
            <span className="absolute right-3 top-3 rounded-full bg-black/50 px-2.5 py-1 text-[10px] font-semibold text-white backdrop-blur-md">
              {tag}
            </span>
          )}

          <div className="absolute bottom-3 left-3 flex items-center gap-1 rounded-full glass-pill px-2.5 py-1">
            <Leaf className="h-3 w-3 text-brand-fresh" />
            <span className="text-[10px] font-semibold text-white">{product.freshnessScore}% fresh</span>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <div className="mt-auto pt-3">
            {cartQuantity > 0 ? (
              <div
                className="flex h-11 items-center justify-between rounded-full bg-brand-dark px-1.5 shadow-md shadow-brand-dark/20"
              ...
              <button
                onClick={handleAdd}
                disabled={!product.inStock}
                className="flex h-11 w-full items-center justify-center gap-1.5 rounded-full border-[1.5px] border-brand-dark/20 text-[13px] font-semibold text-brand-dark transition-all hover:border-brand-dark/40 hover:bg-brand-dark/[0.03] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
              >
