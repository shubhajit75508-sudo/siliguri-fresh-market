...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 glass-strong safe-bottom lg:hidden">
      <div className="flex items-end justify-around px-1 pb-2 pt-1">
        {items.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-[22px] w-[22px]", active && "text-[#E8A440]")} strokeWidth={active ? 2.5 : 2} />
              {item.label}
            </Link>
          );
        })}
        ...
        <button onClick={openCart} className="relative -mt-4 flex flex-col items-center">
          <div className="flex h-[52px] w-[52px] items-center justify-center rounded-full btn-primary">
            <ShoppingBag className="h-5 w-5" strokeWidth={2.5} />
            {count > 0 && (
              <span className="absolute right-0 top-0 -translate-y-1/3 translate-x-1/3 flex h-5 w-5 items-center justify-center rounded-full bg-[#E8A440] text-[10px] font-bold text-[#1A1512]">
                {count}
              </span>
            )}
          </div>
