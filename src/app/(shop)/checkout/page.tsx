      ...
      <div className="mx-auto max-w-6xl px-3 py-5 sm:px-4 sm:py-8">
        ...
        <div className="mb-6 flex items-center gap-2">
          {[
            ...
            <div key={s.n} className="flex items-center gap-2 flex-1 last:flex-none sm:flex-1">
              <div className="flex items-center gap-2">
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold transition-all ${
                  step > s.n ? "bg-brand-fresh text-white" : step === s.n ? "bg-brand-dark text-white ring-2 ring-brand-dark/30" : "bg-gray-100 text-muted"
                }`}>
                  {step > s.n ? <CheckCircle className="h-4 w-4" /> : s.n}
                </div>
                <span className={`hidden sm:inline text-xs font-semibold ${step >= s.n ? "text-brand-dark" : "text-muted"}`}>{s.label}</span>
              </div>
              {i < 2 && <div className={`flex-1 h-0.5 rounded-full ${step > s.n ? "bg-brand-fresh" : "bg-gray-200"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-5 lg:gap-8">
          ...
          <button onClick={() => { setStep(2); window.scrollTo({ top: 0, behavior: "smooth" }); }}
            className={`mt-4 flex w-full items-center justify-center gap-1.5 rounded-2xl border-2 border-border/60 bg-white py-3 text-sm font-semibold text-muted hover:bg-gray-50 transition-all ${step === 3 ? "" : "hidden"}`}>
          ...
          {/* Step 3 — Sidebar */}
          <div className={step === 3 ? "lg:col-span-2" : "hidden lg:hidden"}>
