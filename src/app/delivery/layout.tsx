...
export default function DeliveryLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const orderToAssignment = (o: Record<string, unknown>): DeliveryAssignment => ({
      ...
      address: {
        id: (o.id as string) + "-addr",
        label: "Delivery" as const,
        line1: ((o.address_snapshot as Record<string, unknown>)?.line1 as string) ?? "",
        line2: ((o.address_snapshot as Record<string, unknown>)?.line2 as string) ?? "",
        area: ((o.address_snapshot as Record<string, unknown>)?.area as string) ?? undefined,
        landmark: ((o.address_snapshot as Record<string, unknown>)?.landmark as string) ?? undefined,
        building: ((o.address_snapshot as Record<string, unknown>)?.building as string) ?? undefined,
        flat: ((o.address_snapshot as Record<string, unknown>)?.flat as string) ?? undefined,
        floor: ((o.address_snapshot as Record<string, unknown>)?.floor as string) ?? undefined,
        city: ((o.address_snapshot as Record<string, unknown>)?.city as string) ?? "",
        pincode: ((o.address_snapshot as Record<string, unknown>)?.pincode as string) ?? "",
        lat: (o.address_snapshot as Record<string, unknown>)?.lat as number | undefined,
        lng: (o.address_snapshot as Record<string, unknown>)?.lng as number | undefined,
        isDefault: false,
      },
      items: ((o.items as Record<string, unknown>[]) ?? []).map((i: Record<string, unknown>) => ({ product: { id: (i.product as Record<string, unknown>)?.id as string ?? "", name: (i.product as Record<string, unknown>)?.name as string ?? "", image: (i.product as Record<string, unknown>)?.image as string | undefined, price: (i.product as Record<string, unknown>)?.price as number ?? 0, weightPrices: (i.product as Record<string, unknown>)?.weightPrices as { weight: string; price: number }[] | undefined }, quantity: i.quantity as number, selectedWeight: i.selectedWeight as string | undefined, selectedCut: i.selectedCut as string | undefined, selectedCleaning: i.selectedCleaning as string | undefined, unitPrice: (i.unitPrice as number | undefined) ?? undefined })),
