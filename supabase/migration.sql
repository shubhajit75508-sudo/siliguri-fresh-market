...
-- delivery_locations (real-time GPS tracking, like Blinkit)
...
CREATE POLICY "addresses_user_all" ON public.addresses FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "orders_select" ON public.orders FOR SELECT USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "orders_insert" ON public.orders FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "orders_update_admin" ON public.orders FOR UPDATE USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "order_items_select" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders o WHERE o.id = order_id AND (o.user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'))));
...
CREATE POLICY "coupons_write_admin" ON public.coupons FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "coupons_delete_admin" ON public.coupons FOR DELETE USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "delivery_boys_read" ON public.delivery_boys FOR SELECT USING (true);
CREATE POLICY "delivery_boys_write_admin" ON public.delivery_boys FOR ALL USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "delivery_assignments_select" ON public.delivery_assignments FOR SELECT USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'admin') OR delivery_boy_id = (SELECT id FROM public.delivery_boys WHERE id = delivery_boy_id));

INSERT INTO public.categories (slug, name, description, icon, color, image) VALUES
  ('fish', 'Fresh Fish', 'River-fresh catch daily', 'fish', '#1E88E5', '/categories/fish.jpg'),
  ('chicken', 'Chicken', 'Farm-fresh poultry', 'chicken', '#FF7043', '/categories/chicken.jpg'),
  ('mutton', 'Mutton', 'Premium goat meat', 'mutton', '#8D6E63', '/categories/mutton.jpg'),
  ('pork', 'Pork', 'Fresh pork cuts', 'pork', '#AD3B3B', '/categories/pork.jpg'),
  ('seafood', 'Seafood', 'Ocean delights', 'seafood', '#00ACC1', '/categories/seafood.jpg'),
  ('vegetables', 'Vegetables', 'Farm-fresh greens', 'veg', '#66BB6A', '/categories/vegetables.jpg'),
  ('fruits', 'Fruits', 'Seasonal fruits', 'fruit', '#FFA726', '/categories/fruits.jpg'),
  ('eggs', 'Eggs', 'Farm eggs', 'eggs', '#FFCC80', '/categories/eggs.jpg'),
  ('dairy', 'Dairy', 'Milk & milk products', 'dairy', '#90CAF9', '/categories/dairy.jpg'),
  ('grocery', 'Grocery', 'Daily essentials', 'grocery', '#AB47BC', '/categories/grocery.jpg'),
  ('essentials', 'Essentials', 'Home staples', 'essentials', '#78909C', '/categories/essentials.jpg')
